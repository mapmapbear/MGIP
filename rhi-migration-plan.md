...
- [ ] **step 2**：句柄化 6 个 native culling buffer——`gpuCulling{Object,Stats,Result,Uniform}Buffer` + `external{Object,Meshlet,SceneObject}Buffer`（对应 binding 0/2/3/6/7/8；binding 1/4 已有 `getGPUCulling{Indirect,DrawCount}BufferRHIHandle`）。加法中性，参照 Wave 5 `rebindFrameBufferHandle`。
