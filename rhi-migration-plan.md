...
- [ ] **step 3**：`createGPUCullingResources` → `createArgumentLayout`（9 binding，binding 5 `arrayCount=LDepthPyramidMaxMips`、type=sampledTexture）+ `createArgumentTable`；`updateGPUCullingDescriptorSet` → `updateArgumentTable`（9 写：buffer 用 BufferHandle，binding 5 用 LDepthPyramidMaxMips 个 `arrayElement`+`textureView=getDepthPyramidMipView(i)`+`imageLayout=General`）。**改行为，需 RenderDoc 验证遮挡剔除结果一致**。
- [ ] **step 4**：gpuCulling compute pipeline → RHI `createPipeline`（移除 native `vkCreateComputePipelines` + `m_gpuCullingPipeline` 走 registerExternal 的桥接）。
- [ ] **step 5**：删除 `m_device.gpuCullingSetLayout/gpuCullingPipelineLayout/gpuCullingDescriptorSets/gpuCullingBindGroups` 等 native 成员 + `registerExternalBindGroup`/`updateGPUCullingDescriptorSet`/native 描述符池占用。
