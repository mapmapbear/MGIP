...
### gpuCulling 子系统迁移（起点，步骤已 scoped）
- [x] **step 1（已提交 `d2fecac`）**：RHI 前置——`ArgumentWrite` 加 `imageLayout`（默认 `ShaderRead`，可指定 `General`），后端 `updateArgumentTable` 的 sampledTexture 用它（行为中性）。解锁 binding 5（GENERAL 的 depth-pyramid mip 数组）；顺带可修 SSR 的 GENERAL 布局问题。
- [x] **step 2（已提交 `ccfd5a5`）**：句柄化 6 个 binding 对应的 native culling buffer。`FrameUserData` 加 7 个 `BufferHandle`（gpuCulling{Object,Stats,Result,Uniform}BufferRHI + external{Object,Meshlet,SceneObject}BufferRHI）；`rebindFrameBufferHandle` 加 `VkBuffer` 重载（external 是裸 `VkBuffer`），原重载委托。3 处 rebind：uniform 在 `createFrameSubmission`（init 一次）、object/stats/result 在 `ensureGPUCullingBuffers`（realloc）、external 3 个在 `updateGPUCullingBuffers`（每帧 swap）。加法中性（句柄仅注册未消费），已实机验证。
- [x] **step 3（已提交 `2c0c8b5`）**：`createGPUCullingResources` 用 `createArgumentLayoutFromEntries`（9 binding，binding 5 `sampledImage` 数组 `LDepthPyramidMaxMips`）+ per-frame `createArgumentTable`/`createBindGroup`；`updateGPUCullingDescriptorSet` → `updateArgumentTable`（binding 0/7/8 按 useExternal 选 step 2 句柄，binding 5 用 `arrayElement`+`getDepthPyramidMipView(i)`+`imageLayout=General`）。`getGPUCullingDescriptorSetOpaque` 改用 `resolveArgumentTable` 取底层 native set，保留 HiZ `bindForCulling` 对 binding 5 的双写与诊断。已实机+RenderDoc 验证遮挡剔除一致。
- [x] **step 4+5（已提交 `67e0401`）**：gpuCullingPipelineLayout native `VkPipelineLayout` → RHI `unique_ptr<VulkanPipelineLayout>`（lowering：set 0 = culling ArgumentLayout 的 descriptor set layout）；`createGPUCullingPipeline` 裸 `vkCreateComputePipelines` → `rhi::vulkan::createComputePipeline` 自由函数（仍 registerPipeline 桥接）；删除 step 3 后已死的 `gpuCullingSetLayout`/`gpuCullingDescriptorSets` 成员+teardown；teardown 改 unique_ptr deinit/reset。`registerExternalBindGroup` 保留（shadowCulling 仍用）。**gpuCulling 子系统 native Vk* 成员已全清。**

### 其余子系统（同模式，后续）
- [x] shadowCulling（已提交 `430bafb`）：与 gpuCulling 同构迁移完成。补 `shadowCullingObjectBufferRHI`（binding 0）；2-binding ArgumentLayout + ArgumentTable；pipeline layout → RHI `VulkanPipelineLayout`（带 `ShadowCullPushConstants` push constant range，lowering set 0）；`createShadowCullingPipeline` → `rhi::vulkan::createComputePipeline`；getter 走 resolveArgumentTable/getNativeHandle；删 `shadowCullingSetLayout`/`shadowCullingDescriptorSets`。native Vk* 成员已清。
