...
echo Compressing DDS textures under: "%RESOURCE_DIR%"
if defined QUALITY_ARG (
  echo Compression quality preset: %~1
  "%CONVERTER%" "%RESOURCE_DIR%" "%RESOURCE_DIR%" --format bc7 --input-ext .dds %QUALITY_ARG%
) else (
  echo Compression quality preset: normal ^(CUDA if available^)
  "%CONVERTER%" "%RESOURCE_DIR%" "%RESOURCE_DIR%" --format bc7 --input-ext .dds
)
