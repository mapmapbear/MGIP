...
void GPUDrivenClusteredLightCullingPass::execute(const PassContext& context) const
{
  ...
  const rhi::ArgumentTableHandle argumentTable = m_renderer->getLightCoarseCullingArgumentTable(context.frameIndex);
  if(context.params->debugOptions.enableClusteredLighting && !pipeline.isNull() && !argumentTable.isNull())
  {
    ...
    if(!statsBuffer.isNull())
    {
      ...
      context.commandBuffer->barrier(rhi::StageFlags::transfer, rhi::StageFlags::compute, rhi::HazardFlags::bufferWrites);
    }
    ...
    context.commandBuffer->barrier(rhi::StageFlags::compute, rhi::StageFlags::fragmentShader, rhi::HazardFlags::bufferWrites);
  }
