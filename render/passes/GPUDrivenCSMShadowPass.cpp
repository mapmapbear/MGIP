...
namespace demo
{
	...
	void GPUDrivenCSMShadowPass::execute(const PassContext& context) const
	{
		...
		for (uint32_t cascadeIndex = 0; cascadeIndex < cascadeCount; ++cascadeIndex)
		{
			if (useShadowCulling)
			{
				...
				if (!computePipeline.isNull() && !computeTable.isNull() && indirectBuffer != 0)
				{
					...
					cenc->dispatch(rhi::DispatchDesc{
						(sceneView->shadowPackedMeshCount + shaderio::LGPUCullingThreadCount - 1u)
			...
			const rhi::ArgumentTableHandle drawTable = m_renderer->getCSMShadowMDIDrawArgumentTable(
			...
			if (!drawTable.isNull())
			{
				...
				DrawStreamRecorder::recordIndexedIndirect(*enc, DrawStreamRecorder::IndexedIndirectRecordDesc{
					                                          ...
					                                          getShadowCullingIndirectBufferRHIHandle(frameIndex),
					                                          .offset = 0,
