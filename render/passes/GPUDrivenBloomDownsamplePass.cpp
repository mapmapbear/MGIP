...
namespace demo
{
	...
	void GPUDrivenBloomDownsamplePass::execute(const PassContext& context) const
	{
		...
		const auto transitionStep = [&](const BloomStep& step, rhi::ResourceState before, rhi::ResourceState after)
		{
			...
			// Special resource boundary: each bloom substep alternates between sampling
			// and color attachment writes on explicit mip-chain resources.
			context.commandBuffer->resourceBarrier(&barrier, 1, nullptr, 0);
		};
		const auto renderStep = [&](const BloomStep& step)
		{
			if (step.image.isNull() || step.view.isNull() || step.extent.width == 0u || step.extent.height == 0u)
			{
				return;
			}

			transitionStep(step, rhi::ResourceState::General, rhi::ResourceState::ColorAttachment);
			...
			if (!step.pipeline.isNull() && !inputTable.isNull())
			{
				...
				const rhi::ArgumentTableHandle sceneTable = m_renderer->getLightingSceneArgumentTable(
					context.frameIndex);
			...
			transitionStep(step, rhi::ResourceState::ColorAttachment, rhi::ResourceState::General);
		};

		renderStep(BloomStep{
			m_renderer->getBloomQuarterImage(), m_renderer->getBloomQuarterView(), m_renderer->getBloomQuarterExtent(),
			m_renderer->getBloomHalfExtent(), kPassBloomQuarterHandle, downsamplePipeline, 5u, 0u, 1.0f
		});
		renderStep(BloomStep{
			m_renderer->getBloomEighthImage(), m_renderer->getBloomEighthView(), m_renderer->getBloomEighthExtent(),
			m_renderer->getBloomQuarterExtent(), kPassBloomEighthHandle, downsamplePipeline, 6u, 0u, 1.0f
		});
		renderStep(BloomStep{
			m_renderer->getBloomSixteenthImage(), m_renderer->getBloomSixteenthView(),
			m_renderer->getBloomSixteenthExtent(), m_renderer->getBloomEighthExtent(), kPassBloomSixteenthHandle,
			downsamplePipeline, 13u, 0u, 1.0f
		});
		renderStep(BloomStep{
		...
		renderStep(BloomStep{
			m_renderer->getBloomOutputImage(), m_renderer->getBloomOutputView(), m_renderer->getBloomOutputExtent(),
			m_renderer->getBloomQuarterExtent(), kPassBloomOutputHandle, upsamplePipeline, 5u, 18u, 1.0f
		});
