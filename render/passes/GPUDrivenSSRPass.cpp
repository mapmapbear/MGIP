...
void GPUDrivenSSRPass::execute(const PassContext& context) const
{
  ...
  const uint64_t cameraBuffer = (static_cast<uint64_t>(cameraBufferHandle.generation) << 32u) | cameraBufferHandle.index;
  ...
  context.commandBuffer->barrier(rhi::StageFlags::compute, rhi::StageFlags::fragmentShader, rhi::HazardFlags::textureWrites);
}
