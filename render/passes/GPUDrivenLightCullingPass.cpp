...
namespace demo
{
	...
	PassNode::HandleSlice<PassResourceDependency> GPUDrivenLightCullingPass::getDependencies() const
	{
		static const std::array<PassResourceDependency, 7> dependencies = {
			...
			PassResourceDependency::buffer(kPassPointLightCoarseBoundsHandle, ResourceAccess::write,
			                               rhi::ShaderStage::compute),
			PassResourceDependency::buffer(kPassSpotLightCoarseBoundsHandle, ResourceAccess::write,
			                               rhi::ShaderStage::compute),
		...
		if (context.params->cameraUniforms != nullptr && !argumentTable.isNull())
		{
			...
			const auto dispatchLightKernel = [&](PipelineHandle handle, uint32_t lightCount)
			{
				...
				enc->dispatch(rhi::DispatchDesc{
					(lightCount + kLightCoarseCullingThreadCount - 1u) / kLightCoarseCullingThreadCount, 1u, 1u
				});
			};
