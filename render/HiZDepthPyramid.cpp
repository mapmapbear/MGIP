...
namespace demo
{
	namespace
	{
		...
		utils::Buffer createUniformBuffer(VmaAllocator allocator)
		{
			...
			VK_CHECK(
				vmaCreateBuffer(allocator, &bufferInfo, &allocationCreateInfo, &buffer.buffer, &buffer.allocation, &
					allocationInfo));
	...
	void HiZDepthPyramid::shutdown()
	{
		...
		for (PerFrameResources& frameResources : m_perFrame)
		{
			...
			{
				vmaDestroyBuffer(m_allocator, frameResources.uniformBuffer.buffer,
				                 frameResources.uniformBuffer.allocation);
