...
namespace demo
{
	namespace
	{
		                                      ...
		                                      std::filesystem::path* outPath = nullptr)
		{
			if (imageData.isKtx2 && !imageData.ktx2Data.empty())
			{
				if (outPath != nullptr)
				{
					*outPath = imageData.uri.empty()
						           ? std::filesystem::path("<embedded KTX2>")
						           : std::filesystem::path(imageData.uri);
				}
		...
		[[nodiscard]] shaderio::DrawUniforms buildShadowDrawUniforms(const MeshRecord& mesh,
		                                                             const SceneUploadResult::SceneDrawRecord&
		                                                             drawRecord)
		{
		                                                ...
		                                                OverlayCircle& outCircle)
		{
			...
			ImVec2 centerPoint{};
			ImVec2 radiusPoint{};
			if (!projectPoint(worldCenter, centerPoint) || !
				projectPoint(worldCenter + right * worldRadius, radiusPoint))
			{
				return false;
			}

			outCircle.center = centerPoint;
			outCircle.radius = std::max(1.0f, std::sqrt(
				                            (radiusPoint.x - centerPoint.x) * (radiusPoint.x - centerPoint.x)
	...
	static rhi::ShaderReflectionData buildRasterShaderReflection()
	{
		...
		reflection.resourceBindings = {
			rhi::ShaderResourceBinding{
				"textures", rhi::ShaderResourceType::sampler, rhi::DescriptorType::combinedImageSampler,
			...
			rhi::ShaderResourceBinding{
				"sceneInfo", rhi::ShaderResourceType::uniformBuffer, rhi::DescriptorType::uniformBufferDynamic,
	...
	void RenderDevice::shutdown(rhi::Surface& surface)
	{
		...
		m_materials.texturePool.forEachActive(
			...
			{
				texturesToDestroy.push_back(handle);
			});
		...
		m_materials.materialPool.forEachActive(
			[&](MaterialHandle handle, const MaterialResources::MaterialRecord&)
			{
				materialsToDestroy.push_back(handle);
			});
	...
	ImTextureID RenderDevice::getViewportTextureID(TextureHandle handle) const
	{
		...
		{
			LOGW("RenderDevice::getViewportTextureID rejected stale/invalid texture handle (index=%u generation=%u)",
			     handle.index,
			     handle.generation);
			return ImTextureID{};
		}
	...
	RuntimeProfileSnapshot RenderDevice::getRuntimeProfileSnapshot() const
	{
		...
		snapshot.cpuPassDurationsMs.assign(m_passGpuProfile.latestCpuPassDurationsMs.begin(),
		                                   m_passGpuProfile.latestCpuPassDurationsMs.begin() + static_cast<ptrdiff_t>(
			                                   safePassCount));
		snapshot.gpuPassDurationsMs.assign(m_passGpuProfile.latestPassDurationsMs.begin(),
		                                   m_passGpuProfile.latestPassDurationsMs.begin() + static_cast<ptrdiff_t>(
			                                   safePassCount));
	...
	bool RenderDevice::prepareFrameResources()
	{
		...
		{
			demo::profiling::ScopedCpuRange rebuildSwapchainRange(
				"PrepareFrameResources.RebuildSwapchainDependentResources");
	...
	void RenderDevice::updateLightingUniformBuffer(uint32_t frameIndex,
	...
	void RenderDevice::updateLightCullingUniformBuffer(uint32_t frameIndex,
	...
	void RenderDevice::updateGPUCullingBuffers(uint32_t frameIndex, const RenderParams& params)
	{
		...
		const uint32_t objectCount = useExternalPersistentObjects
			                             ...
			                             : (params.gltfModel != nullptr
				                                ? static_cast<uint32_t>(params.gltfModel->meshes.size())
				                                : 0u);
	...
	void RenderDevice::updateShadowCullingArgumentTable(uint32_t frameIndex)
	{
		...
		PerFrameResources::FrameUserData& frameUserData = m_perFrame.frameUserData[frameIndex];
		if (frameUserData.shadowCullingObjectBufferRHI.isNull() || frameUserData.shadowCullingIndirectBufferRHI.
			isNull())
		{
			return;
		}
		...
		const std::array<rhi::ArgumentWrite, 2> writes{
			{
				rhi::ArgumentWrite{
					.binding = 0, .type = rhi::ArgumentType::storageBuffer,
					.buffer = frameUserData.shadowCullingObjectBufferRHI
				},
				rhi::ArgumentWrite{
	...
	void RenderDevice::updateShadowCullingDrawDataArgumentTable(uint32_t frameIndex)
	{
		...
		{
			const rhi::ArgumentTableHandle drawArgumentTableHandle = getCSMShadowMDIDrawArgumentTable(
				frameIndex, cascadeIndex);
	...
	void RenderDevice::ensureGBufferMdiDrawDataBuffer(PerFrameResources::FrameUserData& frameUserData,
	...
	void RenderDevice::ensureMdiDrawDataBuffer(PerFrameResources::FrameUserData& frameUserData,
	                                           uint32_t requiredDrawCount)
	{
		const uint32_t requiredCapacity = std::max(requiredDrawCount, 1u);
	...
	void RenderDevice::ensureDepthMdiDrawDataBuffer(PerFrameResources::FrameUserData& frameUserData,
	                                                uint32_t requiredDrawCount)
	{
		const uint32_t requiredCapacity = std::max(requiredDrawCount, 1u);
	...
	void RenderDevice::updateShadowCullingBuffers(uint32_t frameIndex, const RenderParams& params)
	{
		...
		{
			if (params.gpuDrivenSceneView->meshHandles == nullptr || params.gpuDrivenSceneView->shadowPackedMeshes ==
				nullptr)
			{
				return;
			}
		...
		std::vector<shaderio::GPUCullIndirectCommand>
			bootstrapCommands(objectCount, shaderio::GPUCullIndirectCommand{});
		...
		for (uint32_t meshIndex = 0; meshIndex < objectCount; ++meshIndex)
		{
			...
			const glm::vec4 casterBounds = packedMesh.boundsSphere.w > 0.0f
				                               ...
				                               : glm::vec4(meshRecord->worldBoundsCenter,
				                                           meshRecord->worldBoundsRadius);
			...
			drawData[meshIndex] = packedMesh.boundsSphere.w > 0.0f
				                      ? packedMesh.drawData
	...
	void RenderDevice::cacheGPUCullingStats(uint32_t frameIndex, bool readOverlayObjects)
	{
		...
		const size_t objectCount = std::min<size_t>(m_lastGPUCullingStats.totalCount,
		                                            frameUserData.gpuCullingResults.size());
	...
	void RenderDevice::resolvePassGpuProfileResults(uint32_t frameIndex)
	{
		...
		for (size_t passIndex = 0; passIndex < frame.passDurationsMs.size(); ++passIndex)
		{
			...
			frame.passDurationsMs[passIndex] = static_cast<double>(delta) * static_cast<double>(m_passGpuProfile.
				timestampPeriodNs) * 1e-6;
			anyValidPass = true;
		}
	...
	{
		if (context.commandBuffer == nullptr || context.frameIndex >= m_passGpuProfile.frames.size() || m_passGpuProfile
			.queryCount == 0)
		{
			return;
		}
	...
	void RenderDevice::PassProfilingHooks::beforePass(const PassContext& context, const PassNode& pass,
	...
	void RenderDevice::PassProfilingHooks::afterPass(const PassContext& context, const PassNode& pass,
	                                                 uint32_t passIndex) const
	{
		(void)pass;
		if (renderer != nullptr)
	...
	void RenderDevice::createGPUCullingResources()
	{
		...
		const std::vector<rhi::ArgumentBinding> layoutEntries{
			...
			makeArgumentBinding(5, rhi::BindlessResourceType::sampledImage, shaderio::LDepthPyramidMaxMips,
			                    rhi::ResourceVisibility::compute),
	...
	void RenderDevice::createShadowCullingResources()
	{
		...
		const rhi::ArgumentLayoutHandle shadowLayout =
			createArgumentLayoutFromBindings(layoutEntries, "shadow-culling");
	...
	void RenderDevice::updateGPUCullingArgumentTable(uint32_t frameIndex)
	{
		...
		writes.push_back(rhi::ArgumentWrite{
			.binding = 6, .type = rhi::ArgumentType::uniformBuffer, .buffer = frameUserData.gpuCullingUniformBufferRHI,
			.size = sizeof(shaderio::GPUCullingUniforms)
		});
		writes.push_back(rhi::ArgumentWrite{
			.binding = 7, .type = rhi::ArgumentType::storageBuffer, .buffer = meshletHandle
		});
		writes.push_back(rhi::ArgumentWrite{
	...
	void RenderDevice::drawFrame(rhi::CommandBuffer& cmdBuffer, const RenderParams& params, PassExecutor& passExecutor)
	{
...

		{
			...
			updateLightingUniformBuffer(currentFrameIndex,
		...
		else
		{
			...
			cameraData.view = glm::lookAt(glm::vec3(0.0f, 0.0f, 3.0f), glm::vec3(0.0f, 0.0f, 0.0f),
	...
	void RenderDevice::DebugDrawList::addSphere(const glm::vec3& center, float radius, const glm::vec4& color,
	                                            uint32_t segments)
	{
	...
	void RenderDevice::DebugDrawList::addArrow(const glm::vec3& origin, const glm::vec3& direction, float length,
	...
	RenderDevice::Aabb RenderDevice::computeSceneBounds(const GltfUploadResult* gltfModel,
	                                                    const GPUDrivenSceneView* gpuDrivenSceneView) const
	{
		...
		if (gpuDrivenSceneView != nullptr && gpuDrivenSceneView->meshHandles != nullptr && gpuDrivenSceneView->
			meshHandleCount > 0u)
		{
	...
	void RenderDevice::ensureTestPointLights(const RenderParams& params)
	{
		...
		if (m_frameLightingState.sceneBounds.valid && params.cameraUniforms != nullptr)
		{
			...
			const glm::vec3 focus = glm::clamp(cameraPosition + cameraForward * focusDistance, sceneMinBounds,
			                                   sceneMaxBounds);
		...
		if (m_testPointLights.empty())
		{
			...
			for (uint32_t i = 0; i < kTestPointLightCount; ++i)
			{
				...
				motion.amplitude = boundsSize * glm::vec3(amplitudeDistribution(rng), amplitudeDistribution(rng),
	...
	std::array<glm::vec3, 8> RenderDevice::computePerspectiveFrustumCorners(
	...
	shaderio::GPUCullingUniforms RenderDevice::buildGPUCullingUniforms(const RenderParams& params,
	                                                                   uint32_t objectCount) const
	{
		...
		const uint32_t mipCount =
			std::min<uint32_t>(m_swapchainDependent.sceneResources.getDepthPyramidMipCount(),
			                   shaderio::LDepthPyramidMaxMips);
		...
		uniforms.cullingControls = glm::vec4(params.debugOptions.enableGPUFrustumCulling ? 1.0f : 0.0f,
		                                     ...
		                                     (meshletCulling && params.debugOptions.enableGPUMeshletConeCulling)
			                                     ? 1.0f
			                                     : 0.0f);
	...
	shaderio::ShadowCullPushConstants RenderDevice::buildShadowCullPushConstants(
	...
	RenderDevice::FrameLightingState RenderDevice::buildFrameLightingState(const RenderParams& params) const
	{
		...
		state.lightParams.iblDebugInfo = glm::vec4(static_cast<float>(params.debugOptions.iblDebugMode), 0.0f, 0.0f,
		                                           0.0f);
	...
	void RenderDevice::buildDebugDrawList(const RenderParams& params)
	{
		...
		{
			m_debugDrawList.addFrustum(m_frameLightingState.shadowFrustumCorners,
			                           glm::vec4(0.95f, 0.75f, 0.20f, 0.90f));
		}
		...
		{
			m_debugDrawList.addArrow(m_frameLightingState.lightAnchor, glm::normalize(params.lightSettings.direction),
			                         6.0f,
		...
		if (params.debugOptions.enablePointLights && params.debugOptions.showPointLights)
		{
			...
			if (m_frameLightingState.sceneBounds.valid)
			{
				const glm::vec3 boundsSize = glm::max(
					m_frameLightingState.sceneBounds.max - m_frameLightingState.sceneBounds.min,
					glm::vec3(1.0f));
	...
	void RenderDevice::createPrebuiltGraphicsPipelineVariants()
	{
		...
		// Create GBuffer pipeline layout with uniform buffer bindings
		{
			...
			std::vector<rhi::ArgumentBinding> cameraLayoutEntries{
				makeArgumentBinding(shaderio::LBindCamera, rhi::BindlessResourceType::uniformBufferDynamic, 1,
				                    rhi::ResourceVisibility::allGraphics),
				makeArgumentBinding(shaderio::LBindLighting, rhi::BindlessResourceType::uniformBuffer, 1,
				                    rhi::ResourceVisibility::fragment),
				makeArgumentBinding(shaderio::LBindLightCulling, rhi::BindlessResourceType::uniformBuffer, 1,
				                    rhi::ResourceVisibility::compute),
				makeArgumentBinding(shaderio::LBindPostProcess, rhi::BindlessResourceType::uniformBufferDynamic, 1,
				                    rhi::ResourceVisibility::fragment),
			};
			...
			const rhi::ArgumentLayoutHandle cameraLayout = createArgumentLayoutFromBindings(
				cameraLayoutEntries, "gbuffer-camera");
				...
				const std::array<rhi::ArgumentWrite, 4> cameraWrites{
					{
						rhi::ArgumentWrite{
							...
							.buffer = fud.transientBufferRHI, .offset = 0, .size = sizeof(shaderio::CameraUniforms)
						},
						rhi::ArgumentWrite{
							.binding = shaderio::LBindLighting, .type = rhi::ArgumentType::uniformBuffer,
							.buffer = fud.lightingBufferRHI, .offset = 0, .size = sizeof(shaderio::LightingUniforms)
						},
						rhi::ArgumentWrite{
							.binding = shaderio::LBindLightCulling, .type = rhi::ArgumentType::uniformBuffer,
							.buffer = fud.lightCullingBufferRHI, .offset = 0,
							.size = sizeof(shaderio::LightCullingUniforms)
						},
						rhi::ArgumentWrite{
							.binding = shaderio::LBindPostProcess, .type = rhi::ArgumentType::uniformBuffer,
							.buffer = fud.transientBufferRHI, .offset = 0, .size = sizeof(shaderio::PostProcessUniforms)
						},
					}
				};
			...
			std::vector<rhi::ArgumentBinding> drawLayoutEntries{
				makeArgumentBinding(shaderio::LBindDrawModel, rhi::BindlessResourceType::uniformBufferDynamic, 1,
				                    rhi::ResourceVisibility::vertex | rhi::ResourceVisibility::fragment),
			};
			const rhi::ArgumentLayoutHandle drawLayout = createArgumentLayoutFromBindings(
				drawLayoutEntries, "gbuffer-draw");
			for (uint32_t i = 0; i < m_perFrame.frameUserData.size(); ++i)
			{
				PerFrameResources::FrameUserData& fud = m_perFrame.frameUserData[i];
				...
				const rhi::ArgumentWrite drawWrite{
			...
			std::vector<rhi::ArgumentBinding> shadowMdiDrawLayoutEntries{
				makeArgumentBinding(shaderio::LBindDrawModelMdi, rhi::BindlessResourceType::storageBuffer, 1,
				                    rhi::ResourceVisibility::vertex | rhi::ResourceVisibility::fragment),
			};
			const rhi::ArgumentLayoutHandle mdiDrawLayout = createArgumentLayoutFromBindings(
				shadowMdiDrawLayoutEntries, "mdi-draw");
		...
		// Create depth prepass pipelines (Opaque + AlphaTest variants)
		{
			...
			const std::array<rhi::VertexAttributeDesc, 4> depthAttributes{
				{
					{
						.location = shaderio::LVGltfPosition, .binding = 0,
						.format = rhi::VertexFormat::r32g32b32Sfloat, .offset = 0
					},
					{
						.location = shaderio::LVGltfNormal, .binding = 0, .format = rhi::VertexFormat::r32g32b32Sfloat,
						.offset = 12
					},
					{
						.location = shaderio::LVGltfTexCoord, .binding = 0, .format = rhi::VertexFormat::r32g32Sfloat,
						.offset = 24
					},
					{
						.location = shaderio::LVGltfTangent, .binding = 0,
						.format = rhi::VertexFormat::r32g32b32a32Sfloat, .offset = 32
					},
				}
			};
		...
		// Create Debug line pipeline
		{
			...
			const std::array<rhi::VertexBindingDesc, 1> debugBindings{
				{
					{
						.binding = 0, .stride = sizeof(shaderio::DebugLineVertex),
						.inputRate = rhi::VertexInputRate::perVertex
					}
				}
			};
			const std::array<rhi::VertexAttributeDesc, 2> debugAttributes{
				{
					{
						.location = shaderio::LVDebugPosition, .binding = 0,
						.format = rhi::VertexFormat::r32g32b32Sfloat,
						.offset = static_cast<uint32_t>(offsetof(shaderio::DebugLineVertex, position))
					},
					{
						.location = shaderio::LVDebugColor, .binding = 0,
						.format = rhi::VertexFormat::r32g32b32a32Sfloat,
	...
	void RenderDevice::createMaterialArgumentTable()
	{
		...
		const rhi::ArgumentLayoutHandle materialLayout = createArgumentLayoutFromBindings(
			layoutEntries, "material-texture-bind-group");
		for (uint32_t frameIndex = 0; frameIndex < frameCount; ++frameIndex)
		{
		...
		m_materials.materialArgumentTable =
			m_materials.materialArgumentTables.empty()
				? rhi::ArgumentTableHandle{}
				: m_materials.materialArgumentTables.front();
	}
	...
	void RenderDevice::createGraphicsArgumentTables()
	{
		...
		{
			std::vector<rhi::ArgumentBinding> layoutEntries{
				makeArgumentBinding(kSceneBindlessInfoIndex, rhi::BindlessResourceType::uniformBuffer, 1,
				                    rhi::ResourceVisibility::allGraphics)
			};

			const rhi::ArgumentLayoutHandle sceneLayout = createArgumentLayoutFromBindings(
				layoutEntries, "scene-dynamic-bind-group");
			for (uint32_t i = 0; i < m_perFrame.frameUserData.size(); ++i)
			{
	...
	{
		if (frameIndex >= m_materials.materialArgumentTables.size() || frameIndex >= m_materials.
			materialArgumentTableGenerations.size())
		{
			return;
		}
		...
		if (const MaterialResources::MaterialRecord* placeholderMaterial = tryGetMaterial(
			m_materials.sampleMaterials[0]))
		{
			if (const MaterialResources::TextureHotData* placeholderTexture = tryGetTextureHot(
				placeholderMaterial->sampledTexture))
			{
		...
		if (!writes.empty())
		{
			updateArgumentTable(m_materials.materialArgumentTables[frameIndex], writes.data(),
			                    static_cast<uint32_t>(writes.size()));
		}
	...
	rhi::ArgumentLayoutHandle RenderDevice::createArgumentLayoutFromBindings(
		std::span<const rhi::ArgumentBinding> bindings,
	...
	rhi::ArgumentTableHandle RenderDevice::createPersistentArgumentTable(
		rhi::ArgumentLayoutHandle layout, const char* debugName)
	{
	                                                      ...
	                                                      rhi::CommandBuffer& cmd)
	{
		...
		meshBatchUpload.init(*m_device.device,
		                     static_cast<uint64_t>(estimatedVertexUploadBytes + estimatedIndexUploadBytes));
		m_meshPool.reserve(estimatedVertexUploadBytes, estimatedIndexUploadBytes, cmd);

		for (const TextureUploadPlan& texturePlan : plan.textures)
		{
			if (texturePlan.textureIndex >= asset.textures.size() || !result.textures[texturePlan.textureIndex].
				isNull())
			{
				continue;
			}
			...
			if (isKtxPayload)
			{
				...
				{
					LOGW("Skipping SceneAsset KTX2 texture %u: %s", texturePlan.textureIndex,
		...
		{
			if (materialPlan.materialIndex >= asset.materials.size() || !result.materials[materialPlan.materialIndex].
				isNull())
			{
				continue;
			}
			...
			if (materialPlan.baseColorTexture >= 0 && static_cast<size_t>(materialPlan.baseColorTexture) < result.
				textures.size())
			{
				record.baseColorTexture = result.textures[materialPlan.baseColorTexture];
				record.sampledTexture = record.baseColorTexture;
			}
			if (materialPlan.metallicRoughnessTexture >= 0 && static_cast<size_t>(materialPlan.metallicRoughnessTexture)
				< result.textures.size())
			{
				record.metallicRoughnessTexture = result.textures[materialPlan.metallicRoughnessTexture];
			}
			if (materialPlan.normalTexture >= 0 && static_cast<size_t>(materialPlan.normalTexture) < result.textures.
				size())
			{
				record.normalTexture = result.textures[materialPlan.normalTexture];
			}
			if (materialPlan.occlusionTexture >= 0 && static_cast<size_t>(materialPlan.occlusionTexture) < result.
				textures.size())
			{
				record.occlusionTexture = result.textures[materialPlan.occlusionTexture];
			}
			if (materialPlan.emissiveTexture >= 0 && static_cast<size_t>(materialPlan.emissiveTexture) < result.textures
				.size())
			{
		...
		for (const MeshUploadPlan& meshPlan : plan.meshes)
		{
			...
			const uint32_t* indexBegin = reinterpret_cast<const uint32_t*>(asset.indexPayload.data() + meshPlan.
				indexSrcOffset);
			SceneMeshData sceneMeshData{
				.interleavedVertexData = std::span<const uint8_t>(vertexBegin,
				                                                  static_cast<size_t>(meshPlan.vertexByteSize)),
			...
			const SceneMaterial* material = mesh.materialIndex < asset.materials.size()
				                                ? &asset.materials[mesh.materialIndex]
				                                : nullptr;
			if (material != nullptr)
			{
				...
				m_meshPool.setMeshMaterialData(meshHandle,
				                               ...
				                               material->baseColorTexture >= 0
					                               ? static_cast<int32_t>(getGltfTextureBaseIndex() + material->
						                               baseColorTexture)
					                               : -1,
		...
		result.drawRecords.reserve(plan.drawCommands.empty()
			                           ? plan.instances.instances.size()
			                           : plan.drawCommands.size());
		...
		{
			for (uint32_t commandIndex = 0; commandIndex < static_cast<uint32_t>(plan.drawCommands.size()); ++
			     commandIndex)
			{
		...
		{
			for (uint32_t instanceIndex = 0; instanceIndex < static_cast<uint32_t>(plan.instances.instances.size()); ++
			     instanceIndex)
			{
				...
				if (instance.materialIndex < asset.materials.size())
				{
					...
					bucket = alphaMode == shaderio::LAlphaBlend
						         ...
						         : (alphaMode == shaderio::LAlphaMask
							            ? SceneDrawBucket::AlphaMask
							            : SceneDrawBucket::Opaque);
				}
	                                        ...
	                                        rhi::CommandBuffer& cmd)
	{
		...
		for (const uint32_t textureIndex : textureIndices)
		{
			...
			const uint32_t mipLevels = hasKtx2Sidecar
				                           ? std::max(ktxTexture.mipLevels, 1u)
		...
		for (const uint32_t textureIndex : textureIndices)
		{
			...
			const GltfImageData* rawImageData = hasSupportedKtx2Sidecar
				                                    ? nullptr
				                                    : findRawImageFallback(model, imageData);
			...
			const uint32_t height = hasSupportedKtx2Sidecar
				                        ? ktxTexture.height
				                        : static_cast<uint32_t>(rawImageData->height);
			const uint32_t requestedMipLevels =
				hasSupportedKtx2Sidecar
					? std::max(ktxTexture.mipLevels, 1u)
					: MipmapGenerator::calculateMipLevelCount(width, height);
			...
			else
			{
				...
				{
					LOGI(
						"Skipping runtime mip generation for raw glTF texture: uri=%s size=%ux%u requestedMipLevels=%u",
		...
		for (const uint32_t materialIndex : materialIndices)
		{
			...
			if (matData.baseColorTexture >= 0 && static_cast<size_t>(matData.baseColorTexture) < ioResult.textures.
				size())
			{
				record.baseColorTexture = ioResult.textures[matData.baseColorTexture];
				record.sampledTexture = record.baseColorTexture;
			}
			if (matData.metallicRoughnessTexture >= 0 && static_cast<size_t>(matData.metallicRoughnessTexture) <
				ioResult.textures.size())
			{
			...
			if (matData.occlusionTexture >= 0 && static_cast<size_t>(matData.occlusionTexture) < ioResult.textures.
				size())
			{
		...
		for (const uint32_t meshIndex : meshIndices)
		{
			...
			if (meshData.materialIndex >= 0 && meshData.materialIndex < static_cast<int>(model.materials.size()))
			{
				...
				m_meshPool.setMeshMaterialData(meshHandle,
				                               ...
				                               matData.baseColorTexture >= 0
					                               ? static_cast<int32_t>(getGltfTextureBaseIndex() + matData.
						                               baseColorTexture)
					                               : -1,
				                               matData.normalTexture >= 0
					                               ? static_cast<int32_t>(getGltfTextureBaseIndex() + matData.
						                               normalTexture)
					                               : -1,
				                               matData.metallicRoughnessTexture >= 0
					                               ? static_cast<int32_t>(getGltfTextureBaseIndex() + matData.
						                               metallicRoughnessTexture)
					                               : -1,
				                               matData.occlusionTexture >= 0
					                               ? static_cast<int32_t>(getGltfTextureBaseIndex() + matData.
						                               occlusionTexture)
					                               : -1,
				                               matData.emissiveTexture >= 0
					                               ? static_cast<int32_t>(getGltfTextureBaseIndex() + matData.
						                               emissiveTexture)
					                               : -1,
	                                              ...
	                                              rhi::CommandBuffer& cmd)
	{
		...
		for (const size_t meshIndex : result.shadowCasterIndices)
		{
			...
			{
				float* dst = reinterpret_cast<float*>(packedVertexData.data() + (static_cast<size_t>(vertexBase) +
					vertexIndex) * 48u);
		...
		result.shadowPackedVertexBuffer =
			createShadowPackedUploadBuffer(cmd, packedVertexBytes, rhi::BufferUsageFlags::vertex,
			                               "ShadowPackedVertexBuffer");
		result.shadowPackedIndexBuffer =
			createShadowPackedUploadBuffer(cmd, packedIndexBytes, rhi::BufferUsageFlags::index,
			                               "ShadowPackedIndexBuffer");
	}
	                                              ...
	                                              rhi::CommandBuffer& cmd)
	{
		                                  ...
		                                  const shaderio::DrawUniforms& drawData)
		{
			...
			const uint32_t* sourceIndices = reinterpret_cast<const uint32_t*>(asset.indexPayload.data() +
				indexByteOffset);
		...
		result.shadowPackedVertexBuffer =
			createShadowPackedUploadBuffer(cmd, packedVertexBytes, rhi::BufferUsageFlags::vertex,
			                               "SceneShadowPackedVertexBuffer");
		result.shadowPackedIndexBuffer =
			createShadowPackedUploadBuffer(cmd, packedIndexBytes, rhi::BufferUsageFlags::index,
			                               "SceneShadowPackedIndexBuffer");
	}
	...
	void RenderDevice::updateLightCoarseCullingResources(uint32_t frameIndex,
	...
	rhi::BufferHandle RenderDevice::getPreviousGPUDrivenPersistentIndirectStreamBufferRHIHandle(
	...
	uint32_t RenderDevice::getPreviousGPUCullingObjectCount(uint32_t currentFrameIndex,
	...
	rhi::ArgumentTableHandle RenderDevice::getCSMShadowMDIDrawArgumentTable(
	...
	void RenderDevice::uploadMDIDrawDataRange(uint32_t frameIndex, uint32_t firstDrawIndex,
	...
	int32_t RenderDevice::getMaterialBaseColorTextureIndex(MaterialHandle materialHandle,
	                                                       const GltfUploadResult* gltfModel) const
	{
		if (!gltfModel)
		{
	...
	RenderDevice::MaterialTextureIndices RenderDevice::getMaterialTextureIndices(
		MaterialHandle materialHandle, const GltfUploadResult* gltfModel) const
	{
