...
namespace demo
{
	namespace
	{
		...
		void appendTextureIndex(std::vector<uint32_t>& indices, uint32_t textureIndex, std::vector<bool>& queued,
		                        const std::vector<bool>& uploaded)
		{
		...
		glm::vec3 computeSceneMeshCenter(const SceneMesh& mesh)
		{
			const glm::vec3 localCenter = 0.5f * (mesh.localBoundsMin + mesh.localBoundsMax);
			return glm::vec3(mesh.exportTransform * glm::vec4(localCenter, 1.0f));
		}

		uint64_t estimateSceneMeshUploadBytes(const MeshUploadPlan& mesh)
		{
			return mesh.vertexByteSize + mesh.indexByteSize;
		}
	} // namespace
	                                    ...
	                                    uint32_t batchMeshBudget)
	{
		...
		std::sort(prioritizedMeshes.begin(), prioritizedMeshes.end(),
		          [](const PrioritizedMesh& a, const PrioritizedMesh& b)
		          {
	...
	void AsyncLoadingCoordinator::begin(const SceneAssetView& asset,
	                                    ...
	                                    uint32_t batchMeshBudget)
	{
		...
		m_queuedTextures.assign(plan.textures.size(), false);
		m_queuedMaterials.assign(plan.materials.size(), false);
		m_queuedMeshes.assign(plan.meshes.size(), false);
		m_queuedInstances.assign(plan.instances.instances.size(), false);
		m_queuedDrawCommands.assign(plan.drawCommands.size(), false);
		m_uploadedTextures.assign(plan.textures.size(), false);
		m_uploadedMaterials.assign(plan.materials.size(), false);
		m_uploadedMeshes.assign(plan.meshes.size(), false);
		m_uploadedInstances.assign(plan.instances.instances.size(), false);
		m_uploadedDrawCommands.assign(plan.drawCommands.size(), false);

		std::vector<bool> referencedTextures(plan.textures.size(), false);
		...
		std::vector<PrioritizedMesh> prioritizedMeshes;
		prioritizedMeshes.reserve(asset.meshes.size());
		for (uint32_t meshIndex = 0; meshIndex < static_cast<uint32_t>(asset.meshes.size()); ++meshIndex)
		{
			const glm::vec3 center = computeSceneMeshCenter(asset.meshes[meshIndex]);
			prioritizedMeshes.push_back(PrioritizedMesh{
				.index = meshIndex,
				.priority = calculatePriority(center, cameraPos),
				.distance = glm::length(center - cameraPos),
		...
		m_progress = {};
		m_progress.texturesTotal = static_cast<uint32_t>(plan.textures.size());
		m_progress.materialsTotal = static_cast<uint32_t>(plan.materials.size());
		m_progress.meshesTotal = static_cast<uint32_t>(plan.meshes.size());
		m_progress.instancesTotal = static_cast<uint32_t>(plan.instances.instances.size());
		m_progress.drawCommandsTotal = static_cast<uint32_t>(plan.drawCommands.size());
		refreshProgress();
	}
	...
	{
		return (m_model != nullptr || m_scenePlan != nullptr) && m_nextMeshCursor < m_sortedMeshIndices.size();
	}
	...
	AsyncLoadingCoordinator::UploadBatch AsyncLoadingCoordinator::takeNextBatch()
	{
		...
		if (m_scenePlanMode)
		{
			...
			while (m_nextMeshCursor < m_sortedMeshIndices.size() && batch.meshIndices.size() < meshBudget)
			{
				...
				const MeshUploadPlan& meshPlan = m_scenePlan->meshes[meshIndex];
				const uint64_t candidateMeshBytes = estimateSceneMeshUploadBytes(meshPlan);
				uint64_t candidateTextureBytes = 0;
				std::vector<uint32_t> candidateTextures;
				std::vector<uint32_t> candidateMaterials;
				std::vector<uint32_t> candidateInstances;
				std::vector<uint32_t> candidateDrawCommands;

				for (const SceneDrawInstance& instance : m_scenePlan->instances.instances)
				{
					...
					if (instance.materialIndex >= 0 && static_cast<size_t>(instance.materialIndex) < m_queuedMaterials.
						size()
						&& !m_queuedMaterials[instance.materialIndex] && !m_uploadedMaterials[instance.materialIndex]
						&& std::find(candidateMaterials.begin(), candidateMaterials.end(),
						             static_cast<uint32_t>(instance.materialIndex))
						== candidateMaterials.end())
					{
						candidateMaterials.push_back(static_cast<uint32_t>(instance.materialIndex));
					}

					if (instance.instanceIndex < m_queuedInstances.size() && !m_queuedInstances[instance.instanceIndex]
						&& !m_uploadedInstances[instance.instanceIndex]
						&& std::find(candidateInstances.begin(), candidateInstances.end(), instance.instanceIndex) ==
						candidateInstances.end())
					{
						candidateInstances.push_back(instance.instanceIndex);
					}

					if (instance.materialIndex < 0 || static_cast<size_t>(instance.materialIndex) >= m_scenePlan->
						materials.size())
					{
						continue;
					}

					const MaterialCreatePlan& material = m_scenePlan->materials[instance.materialIndex];
					const std::array<int32_t, 5> textureRefs = {
						{
					...
					for (const int32_t textureRef : textureRefs)
					{
						...
						const uint32_t textureIndex = static_cast<uint32_t>(textureRef);
						if (textureIndex >= m_queuedTextures.size() || m_queuedTextures[textureIndex] ||
							m_uploadedTextures[textureIndex])
						{
							continue;
						}
						if (std::find(candidateTextures.begin(), candidateTextures.end(), textureIndex) !=
							candidateTextures.end())
						{
							continue;
						}
						candidateTextures.push_back(textureIndex);
						candidateTextureBytes += m_scenePlan->textures[textureIndex].payloadByteSize;
					}
				}

				const bool exceedsTextureBudget = batchTextureBytes + candidateTextureBytes > textureBudgetBytes;
				const bool exceedsMeshBudget = batchMeshBytes + candidateMeshBytes > meshUploadBudgetBytes;
				...
				for (uint32_t textureIndex : candidateTextures)
				{
					appendTextureIndex(batch.textureIndices, textureIndex, m_queuedTextures, m_uploadedTextures);
				}
				for (uint32_t materialIndex : candidateMaterials)
				{
					m_queuedMaterials[materialIndex] = true;
					batch.materialIndices.push_back(materialIndex);
				}
				for (uint32_t instanceIndex : candidateInstances)
				{
					m_queuedInstances[instanceIndex] = true;
					batch.instanceIndices.push_back(instanceIndex);
				}

				for (uint32_t drawIndex = 0; drawIndex < m_scenePlan->drawCommands.size(); ++drawIndex)
				{
					const DrawCommandBuildPlan& draw = m_scenePlan->drawCommands[drawIndex];
					if (draw.meshIndex != meshIndex)
					{
						continue;
					}
					if (drawIndex < m_queuedDrawCommands.size() && !m_queuedDrawCommands[drawIndex] && !
						m_uploadedDrawCommands[drawIndex])
					{
						candidateDrawCommands.push_back(drawIndex);
					}
				}
				for (uint32_t drawIndex : candidateDrawCommands)
				{
					m_queuedDrawCommands[drawIndex] = true;
					batch.drawCommandIndices.push_back(drawIndex);
				}
		...
		while (m_nextMeshCursor < m_sortedMeshIndices.size() && batch.meshIndices.size() < meshBudget)
		{
			...
			if (materialIndex >= 0 && materialIndex < static_cast<int>(m_model->materials.size()))
			{
				...
				auto stageTexture = [&](int textureIndex)
				{
					...
					if (textureSlot >= m_queuedTextures.size() || m_queuedTextures[textureSlot] || m_uploadedTextures[
						textureSlot])
					{
						return;
					}

					if (std::find(candidateTextures.begin(), candidateTextures.end(), textureSlot) != candidateTextures.
						end())
					{
						return;
					}
	...
	void AsyncLoadingCoordinator::markBatchUploaded(const UploadBatch& batch)
	{
		...
		for (uint32_t index : batch.instanceIndices)
		{
			if (index < m_uploadedInstances.size())
			{
				m_uploadedInstances[index] = true;
			}
		}
		for (uint32_t index : batch.drawCommandIndices)
		{
			if (index < m_uploadedDrawCommands.size())
			{
				m_uploadedDrawCommands[index] = true;
			}
		}
	...
	{
		m_progress.texturesLoaded = static_cast<uint32_t>(std::count(m_uploadedTextures.begin(),
		                                                             m_uploadedTextures.end(), true));
		m_progress.materialsLoaded = static_cast<uint32_t>(std::count(m_uploadedMaterials.begin(),
		                                                              m_uploadedMaterials.end(), true));
		m_progress.meshesLoaded = static_cast<uint32_t>(std::count(m_uploadedMeshes.begin(), m_uploadedMeshes.end(),
		                                                           true));
		m_progress.instancesLoaded = static_cast<uint32_t>(std::count(m_uploadedInstances.begin(),
		                                                              m_uploadedInstances.end(), true));
		m_progress.drawCommandsLoaded = static_cast<uint32_t>(std::count(
			m_uploadedDrawCommands.begin(), m_uploadedDrawCommands.end(), true));

		const float textureRatio = m_progress.texturesTotal > 0
			                           ? static_cast<float>(m_progress.texturesLoaded) / static_cast<float>(m_progress.
				                           texturesTotal)
			                           : 1.0f;
		const float materialRatio = m_progress.materialsTotal > 0
			                            ? static_cast<float>(m_progress.materialsLoaded) / static_cast<float>(m_progress
				                            .materialsTotal)
			                            : 1.0f;
		const float meshRatio = m_progress.meshesTotal > 0
			                        ? static_cast<float>(m_progress.meshesLoaded) / static_cast<float>(m_progress.
				                        meshesTotal)
			                        : 1.0f;
		const float instanceRatio = m_progress.instancesTotal > 0
			                            ? static_cast<float>(m_progress.instancesLoaded) / static_cast<float>(m_progress
				                            .instancesTotal)
			                            : 1.0f;
		const float drawRatio = m_progress.drawCommandsTotal > 0
			                        ? static_cast<float>(m_progress.drawCommandsLoaded) / static_cast<float>(m_progress.
				                        drawCommandsTotal)
			                        : 1.0f;

		if (m_scenePlanMode)
		{
			m_progress.progressPercent = textureRatio * 0.25f
				+ materialRatio * 0.10f
				+ meshRatio * 0.25f
				+ instanceRatio * 0.20f
				+ drawRatio * 0.20f;
		...
		m_progress.isComplete = m_progress.texturesLoaded == m_progress.texturesTotal
			...
			&& m_progress.instancesLoaded == m_progress.instancesTotal
			&& m_progress.drawCommandsLoaded == m_progress.drawCommandsTotal;
	}
} // namespace demo
