  ...
  if(m_scenePlanMode)
  {
    ...
    while(m_nextMeshCursor < m_sortedMeshIndices.size() && batch.meshIndices.size() < meshBudget)
    {
      ...
      for(const SceneDrawInstance& instance : m_scenePlan->instances.instances)
      {
        ...
        if(instance.materialIndex >= 0 && static_cast<size_t>(instance.materialIndex) < m_queuedMaterials.size()
           && !m_queuedMaterials[instance.materialIndex] && !m_uploadedMaterials[instance.materialIndex]
           && std::find(candidateMaterials.begin(), candidateMaterials.end(), static_cast<uint32_t>(instance.materialIndex))
        ...
        if(instance.instanceIndex < m_queuedInstances.size() && !m_queuedInstances[instance.instanceIndex]
           ...
           && std::find(candidateInstances.begin(), candidateInstances.end(), instance.instanceIndex) == candidateInstances.end())
        {
          candidateInstances.push_back(instance.instanceIndex);
        }

        if(instance.materialIndex < 0 || static_cast<size_t>(instance.materialIndex) >= m_scenePlan->materials.size())
        {
          continue;
        }
        ...
        for(const int32_t textureRef : textureRefs)
        {
          ...
          if(textureIndex >= m_queuedTextures.size() || m_queuedTextures[textureIndex] || m_uploadedTextures[textureIndex])
          {
            continue;
          }
          if(std::find(candidateTextures.begin(), candidateTextures.end(), textureIndex) != candidateTextures.end())
          {
            continue;
          }
      ...
      for(uint32_t drawIndex = 0; drawIndex < m_scenePlan->drawCommands.size(); ++drawIndex)
      {
        ...
        if(drawIndex < m_queuedDrawCommands.size() && !m_queuedDrawCommands[drawIndex] && !m_uploadedDrawCommands[drawIndex])
        {
...
void AsyncLoadingCoordinator::refreshProgress()
{
  ...
  m_progress.instancesLoaded = static_cast<uint32_t>(std::count(m_uploadedInstances.begin(), m_uploadedInstances.end(), true));
  m_progress.drawCommandsLoaded = static_cast<uint32_t>(std::count(m_uploadedDrawCommands.begin(), m_uploadedDrawCommands.end(), true));
  ...
  const float instanceRatio = m_progress.instancesTotal > 0
                                  ? static_cast<float>(m_progress.instancesLoaded) / static_cast<float>(m_progress.instancesTotal)
                                  : 1.0f;
  const float drawRatio = m_progress.drawCommandsTotal > 0
                              ? static_cast<float>(m_progress.drawCommandsLoaded) / static_cast<float>(m_progress.drawCommandsTotal)
                              : 1.0f;
