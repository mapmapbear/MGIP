                                      ...
                                      const std::vector<shaderio::GPUCullObject>& meshletCullObjects)
{
  ...
  if(meshletUploadCount > 0)
  {
    ...
    VK_CHECK(vmaFlushAllocation(toVmaAllocator(m_allocator), toVmaAllocation(m_meshletDataBuffer.allocation), meshletOffsetBytes, meshletUploadBytes));
  }
  ...
  if(indexUploadCount > 0)
  {
    ...
    VK_CHECK(vmaFlushAllocation(toVmaAllocator(m_allocator), toVmaAllocation(m_meshletIndexBuffer.allocation), indexOffsetBytes, indexUploadBytes));
  }
}
