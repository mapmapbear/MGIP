...
namespace demo
{
	...
	void IBLResources::createImages(rhi::CommandBuffer& rhiCmd, const CreateInfo& createInfo)
	{
		...
		// inferConsumerAccess only emits a conservative memory-read access on the dst side,
	                                                           ...
	                                                           uint32_t variant) const
	{
		...
		const std::array<rhi::RootBindingDesc, 1> rootBindings{{
			rhi::RootBindingDesc{
				...
				.alignment = alignof(GeneratePushConstants),
			},
		}};
		...
		const rhi::ComputePipelineDesc desc{
			...
			rhi::ShaderEntry{
				...
				.library = loadEmbeddedSpirvLibrary(
					*m_rhiDevice, spirvCode, spirvSize, entryPoint),
	...
	void IBLResources::generateIBLMaps(rhi::CommandBuffer& rhiCmd, const CreateInfo& createInfo)
	{
			...
			) -> bool
		{
			...
			encoder->setRootConstants(0, std::as_bytes(std::span{&constants, 1}));
		...
		{
			const rhi::ArgumentTableHandle table = m_rhiDevice->createArgumentTable(rhi::ArgumentTableCreateDesc{.layout = layout, .lifetime = rhi::ArgumentTableLifetime::persistent});
			m_generationArgumentTables.push_back(table);
			if (writeCount > 0)
			{
				m_rhiDevice->updateArgumentTable(table, rhi::ArgumentWriteBatch{writes, writeCount});
			}
			return table;
		};
		...
		m_lutGenerationArgumentLayout = m_rhiDevice->createArgumentLayout(rhi::ArgumentLayoutDesc{
			.bindings = std::span{&lutBinding, 1}, .debugName = "ibl-lut-generation"
		});
