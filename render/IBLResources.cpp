...
namespace demo
{
	...
	void IBLResources::init(rhi::Device& device, uintptr_t backendAllocatorToken, rhi::CommandBuffer& rhiCmd,
	...
	void IBLResources::createImages(rhi::CommandBuffer& rhiCmd, const CreateInfo& createInfo)
	{
		...
		const auto makeView = [&](VkImage image, rhi::TextureFormat format, rhi::ImageViewType viewType,
		                          uint32_t levelCount,
		                          uint32_t layerCount, const char* name) -> rhi::TextureViewHandle
		{
			const rhi::TextureHandle imageHandle = m_rhiDevice->registerExternalTexture(
		...
		VK_CHECK(
			vmaCreateImage(allocator, &cubeInfo, &imageAllocInfo, &prefilteredMap, &prefilteredAllocation, nullptr));
		m_prefilteredMap = {
			reinterpret_cast<uintptr_t>(prefilteredMap),
		...
		VK_CHECK(
			vmaCreateImage(allocator, &irradianceInfo, &imageAllocInfo, &irradianceMap, &irradianceAllocation, nullptr
			));
		m_irradianceMap = {
			reinterpret_cast<uintptr_t>(irradianceMap),
		...
		m_dfgLUT = {
			reinterpret_cast<uintptr_t>(dfgLut),
		...
		imageBarrier(rhiCmd, m_prefilteredMap.handle, cubeRange, rhi::ResourceState::Undefined,
		             rhi::ResourceState::General);
		imageBarrier(rhiCmd, m_irradianceMap.handle, irradianceRange, rhi::ResourceState::Undefined,
		             rhi::ResourceState::General);
	...
	void IBLResources::generateIBLMaps(rhi::CommandBuffer& rhiCmd, const CreateInfo& createInfo)
	{
		...
		if (!createInfo.sourceEnvironmentView.isNull())
		{
			...
			imageBarrier(rhiCmd, m_irradianceMap.handle, colorRange(1, 6), rhi::ResourceState::General,
			             rhi::ResourceState::General);
	...
	{
		imageBarrier(rhiCmd, m_dfgLUT.handle, colorRange(1, 1), rhi::ResourceState::General,
		             rhi::ResourceState::ShaderRead);
		imageBarrier(rhiCmd, m_irradianceMap.handle, colorRange(1, 6), rhi::ResourceState::General,
		             rhi::ResourceState::ShaderRead);
		imageBarrier(rhiCmd, m_prefilteredMap.handle, colorRange(m_maxMipLevel + 1, 6), rhi::ResourceState::General,
		             rhi::ResourceState::ShaderRead);
	}
