...
void updateMappedStorageBuffer(rhi::Device& device, rhi::BufferHandle buffer, std::span<const shaderio::LightData> lights)
{
...
void updateMappedUniformBuffer(rhi::Device& device, rhi::BufferHandle buffer, const shaderio::LightCoarseCullingUniforms& uniforms)
{
  if(buffer.isNull())
  {
    return;
...
void LightResources::init(rhi::Device& device, uintptr_t backendAllocatorToken, const CreateInfo& createInfo)
{
  deinit();
  ...
  for(FrameResources& frame : m_frames)
  {
    ...
    frame.pointCoarseBoundsBuffer = createStorageBuffer(device, pointBoundsBufferSize, rhi::MemoryUsage::gpuOnly);
    frame.spotCoarseBoundsBuffer = createStorageBuffer(device, spotBoundsBufferSize, rhi::MemoryUsage::gpuOnly);
    frame.coarseCullingUniformBuffer = createUniformBuffer(device, coarseUniformBufferSize, rhi::MemoryUsage::cpuToGpu);
  }
}
...
void LightResources::deinit()
{
  if(m_rhiDevice != nullptr)
  {
    for(FrameResources& frame : m_frames)
    {
      ...
      if(!frame.coarseCullingUniformBuffer.isNull()) m_rhiDevice->destroyBuffer(frame.coarseCullingUniformBuffer);
    }
  }
