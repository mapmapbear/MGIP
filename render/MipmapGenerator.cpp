                                      ...
                                      uint32_t mipLevels)
{
  ...
  for(uint32_t mip = 0; mip + 1 < mipLevels; ++mip)
  {
    ...
    const std::array<rhi::TextureBarrier, 2> toBlit{{
        {.texture = image, .before = rhi::ResourceState::General, .after = rhi::ResourceState::TransferSrc, .range = srcRange},
        {.texture = image,
    ...
    const std::array<rhi::TextureBarrier, 2> fromBlit{{
        {.texture = image, .before = rhi::ResourceState::TransferSrc, .after = rhi::ResourceState::General, .range = srcRange},
        {.texture = image,
