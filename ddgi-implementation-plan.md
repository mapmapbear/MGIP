...
- [ ] `tools/sdf_baker` 可在 vcvars64 环境独立构建（cmake --build 指向 tools/sdf_baker target）
...
- [ ] MSVC 构建通过
- [ ] DDGIProbeVolume 构造/析构全程无 Vulkan 原生调用（vkCreate*/vmaCreate* 等不出现在 render/ 代码中）
- [ ] `check_rhi_boundary.py` 零新增
...
- [ ] 完全不调用 Vulkan RT 扩展（vkGetAccelerationStructure*/vkCmdTraceRays* 不出现）
- [ ] `check_rhi_boundary.py` 零新增
